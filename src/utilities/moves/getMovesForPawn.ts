...
const getMovesForPawn = (requiredContext: RequiredContext) => {
  ...
  const isSingleStepOccupied = Boolean(
    board.find((square) => square.location === singleStepLocation).piece
  )
  ...
  if (isStartingPosition) {
    ...
    const isOccupied = Boolean(
      board.find((square) => square.location === doubleStepLocation).piece
    )
  ...
  const rightOptionIsAGoBoi =
    board.some(
      (square) =>
        square.location === rightOption &&
        square.piece &&
        square.piece.color !== player
    ) && rightOption !== singleStepLocation
  const leftOptionIsAGoBoi =
    board.some(
      (square) =>
        square.location === leftOption &&
        square.piece &&
        square.piece.color !== player
    ) && leftOption !== singleStepLocation
